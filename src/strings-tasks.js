...
function removeFirstOccurrences(str, value) {
  ...
  return index >= 0
    ? str.substring(0, index < 0 ? 0 : index) +
...
function sumOfCodes(str) {
  ...
  if (typeof str === 'string') {
    for (let values = 0; values < str.length; values += 1) {
      count += str.charCodeAt(values);
    }
    return count;
  }
  return count;
...
function isPalindrome(str) {
  ...
  return (
    clean.toLowerCase().split('').reverse().join() ===
    clean.toLowerCase().split('').join()
  );
}
...
function invertCase(str) {
  ...
  str.split('').forEach((letter) => {
    if (letter === letter.toUpperCase()) {
      inverts += letter.toLowerCase();
    } else {
      inverts += letter.toUpperCase();
    }
  });
  ...
  str.split('').forEach((letter, index) => {
    if (
      ...
      if (str.charCodeAt(index) >= 65 && str.charCodeAt(index) <= 90) {
        if (str.charCodeAt(index) + 13 <= 90) {
          word += String.fromCharCode(str.charCodeAt(index) + 13);
        } else {
          word += String.fromCharCode(str.charCodeAt(index) + 13 - 26);
        }
      } else if (str.charCodeAt(index) + 13 <= 122) {
        word += String.fromCharCode(str.charCodeAt(index) + 13);
      } else {
        word += String.fromCharCode(str.charCodeAt(index) + 13 - 26);
      }
